...
def _build_parser():
    ...
    parser.add_argument(
        '-s', '--server',
        help=("The server to post results to, e.g. http://localhost:8080/ . "
              "May also be specified in ~/.teuthology.yaml as 'results_server'"),
    )
    parser.add_argument(
        '-n', '--no-save',
        action='store_true',
        help=("By default, when submitting all runs, we remember the last "
              "successful submission in a file called 'last_successful_run'. "
              "Pass this flag to disable that behavior."),
    )
    parser.add_argument(
        '-D', '--dead',
        action='store_true',
