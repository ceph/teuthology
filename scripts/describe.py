...
def _build_parser():
    ...
    parser.add_argument(
        ...
        default='desc',
        metavar='<fields>',
        help='Comma-separated list of fields to include [default: %(default)s]'
    )
    parser.add_argument(
        '--show-facet',
        default='yes',
        choices=['yes', 'no'],
        help='List the facet of each file [default: %(default)s]'
    )
    parser.add_argument(
        '--format',
        default='plain',
    ...
    parser.add_argument(
        ...
        default=0,
        metavar='<jobs>',
        help='List at most this many jobs [default: %(default)s]'
    )
    parser.add_argument(
        '--subset',
        metavar='<index/outof>',
        help=('Instead of listing the entire suite, break the set of jobs into '
              '<outof> pieces (each of which will contain each facet at least '
              'once) and list piece <index>. Listing 0/<outof>, 1/<outof>, '
              '2/<outof> ... <outof>-1/<outof> will list all jobs in the suite '
              '(many more than once).'),
    )
    parser.add_argument(
        '-S', '--seed',
        ...
        default=-1,
        metavar='<seed>',
        help=('Used for pseudo-random tests generation involving facet whose '
              'path ends with $ operator, where negative value used for a '
