        ...
        with open(log_path) as results_log:
            for line in results_log:
                ...
                for key, value in result_conf.items():
                    if value is None:
                        result_conf[key] = self._parse_log_line(line, '{0}:'.format(key))
        if result_conf['subset'] is not None:
            subset = result_conf['subset']
            result_conf['subset'] = tuple(int(i) for i in subset.split('/'))
        if result_conf['seed'] is not None:
            result_conf['seed'] = int(result_conf['seed'])
        return result_conf
