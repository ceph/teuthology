...
def fetch_binaries_for_coredumps(path: str) -> None:
    ...
    def _get_compressed_type(dump_path: str) -> str :
        """
    ...
    def _uncompress_file(dump_path: str, cs_type: str ) -> str :
        """
    ...
    def _get_program_binary(dump_out: str, coredump_path: str) -> str :
        """
    ...
    # Check for Coredumps:
    coredump_path = os.path.join(path, "")
    if os.path.isdir(coredump_path):
        ...
        for dump in os.listdir(coredump_path):
            ...
            # Check the file looks like a coredump
            if _is_elf(dump_out):
                log.info(f"Found ELF core file: {dump}")
                ...
                if _looks_compressed(dump_out):
                    ...
                    try:
                        ...
                        if not uncompressed_path: # is None:
