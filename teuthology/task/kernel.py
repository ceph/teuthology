...
def safe_hard_reboot(remote):
    ...
    """
    log.info(f"Triggering safe hard reboot via SysRq on {remote.shortname}")
    # s = sync, u = remount read-only, b = reboot
    # We use a multi-command string to ensure they execute even if the connection drops
    args = [
        'sudo', 'bash', '-c',
        'echo pci > /sys/kernel/reboot/type && sync && echo s > /proc/sysrq-trigger && echo u > /proc/sysrq-trigger && sleep 2 && echo b > /proc/sysrq-trigger'
    ]
...
def install_and_reboot(ctx, need_install, config):
    ...
    for role, src in need_install.items():
        ...
        if kernel_title.endswith("-highbank"):
            ...
            procs[role_remote.name] = safe_hard_reboot(role_remote)
            continue
