...
def make_parser(argv=None):
    ...
    parser.add_argument(
        ...
        default='beanstalk',
        metavar='<backend>',
        help=("Queue backend name, use prefix '@' to append job config to the "
              'given file path as yaml [default: %(default)s]'),
    )
    parser.add_argument(
        '-n', '--name',
        required=True,
    ...
    parser.add_argument(
        ...
        default='plana',
        metavar='<worker>',
        help='Which worker to use (type of machine) [default: %(default)s]'
    )
    parser.add_argument(
        '-p', '--priority',
        type=int,
        default=1000,
        metavar='<priority>',
        help='Job priority (lower is sooner) [default: %(default)s]'
    )
    parser.add_argument(
        '-N', '--num',
        type=int,
        default=1,
        metavar='<num>',
        help='Number of times to run/queue the job [default: %(default)s]'
    )
    parser.add_argument(
        '--first-in-suite',
        action='store_true',
