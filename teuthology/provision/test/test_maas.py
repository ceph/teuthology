...
class TestMaas(object):
    ...
    @mark.parametrize("response", [b'{}', b'{"locked": "true"}'])
    def test_lock_machine(self, response):
        with patch.multiple(
            "teuthology.provision.maas.MAAS",
            do_request=DEFAULT,
            _get_system_info=DEFAULT,
        ) as local_mocks:
            local_mocks["_get_system_info"].return_value = (
                "ubuntu", "24.04", "abc123"
            )
            local_mocks["do_request"].return_value = self._get_mock_response(
                content=response
            )
            obj = self.kclass(name="name.fqdn")
            if response == b'{}':
                with raises(RuntimeError):
                    obj.lock_machine()
                return
    ...
    @mark.parametrize("response", [b'{}', b'{"locked": "true"}'])
    def test_unlock_machine(self, response):
        self.mocks["m_Remote_hostname"].return_value = "name.fqdn"
        with patch.multiple(
            "teuthology.provision.maas.MAAS",
            do_request=DEFAULT,
    ...
    @mark.parametrize("response", [b'{}', b'{"status_name": "disk erasing"}'])
    ...
    def test_abort_deploy(self):
        with patch.multiple(
            ...
            local_mocks["do_request"].return_value = self._get_mock_response(
                content=b'{"status_name": "aborted"}'
            )
            self.kclass(name="name.fqdn").abort_deploy()

    @mark.parametrize("lock_status, machine_status", [
        ("locked", "ready"), ("unlocked", "ready"), ("unlocked", "releasing"),
    ])
    def test_release(self, lock_status, machine_status):
        with patch.multiple(
            ...
            local_mocks["_wait_for_status"].side_effect = RuntimeError
            with raises(RuntimeError):
                obj.release()
    ...
    def test_get_user_data(self, user_data):
        with patch(
            "teuthology.provision.maas.MAAS._get_system_info"
        ) as maas_machine:
            maas_machine.return_value = ("ubuntu", "22.04", "abc123")
            obj = self.kclass(name="name.fqdn")
            if user_data:
    ...
    def test_verify_installed_os(
        ...
        with patch(
            ...
            maas_machine.return_value = (os_type, os_version, "abc123")
            obj = self.kclass(name="name.fqdn")
            if os_type != remote_os_type:
                with raises(RuntimeError):
